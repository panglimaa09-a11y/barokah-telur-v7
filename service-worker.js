  ...
  if (url.origin === self.location.origin && (url.pathname === "/" || url.pathname.endsWith("/index.html"))) {
    event.respondWith(
      fetch(event.request).then(async response => {
        ...
        const finalResponse = new Response(injected, {status: response.status, statusText: response.statusText, headers});
        caches.open(CACHE).then(cache => cache.put(event.request, finalResponse.clone()));
